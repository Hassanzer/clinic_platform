...
function WithBabyEdit() {
    ...
    const babyDetail=[
        {
            ...
            type:'text',
            name:"babyFirstName",
            value:pData.babyname
        },
        {
            label:'النسب',
            type:'text',
            name:"babyLastName",
            value:pData.babylastname
        },
    ...
    const fetchPatientData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/print/withbaby`,{params:{id:id}})
    ...
    function onChange(e){
     ...
     function handleUpdate(e){
        ...
        const data = pData
        data.id=id
         axios.post('http://localhost:5000/update/baby',data).then((response) => {
             return response
         }).catch(err => err)
    ...
    <div>
      ...
      إستمارة النزيلة في حالة مرافقة الطفل
        ...
        <form method="POST" className='my-5 container px-4 ' onSubmit={handleUpdate} >
            ...
            <div className="row my-3">

                {
                    ...
                    babyDetail.map((input, index)=>(
                        <div key={index} className='col-6'>
                           <div className="row p-0 m-0">
                                
                                <label key={index} htmlFor={input.name} className="col-4 col-form-label fw-bold  my-3">{input.label}</label>
                                <div className="col-8" >
                                    <input onChange={onChange} type={input.type} value={input.value} className="form-control border-primary my-3" id={input.name} name={input.name} required />
                                </div>
                ...
                <div className="my-3">
                        ...
                        <select defaultValue={'DEFAULT'} value={
                            pData.bloodrelationship === 'الأم' ? 'الأم' :
                            pData.bloodrelationship === 'الجدة' ? 'الجدة' :
                            pData.bloodrelationship === 'الخالة' ? 'الخالة' :
                            pData.bloodrelationship === 'العمة' ? 'العمة' :
                            pData.bloodrelationship === 'أخر' ? 'أخر' :'DEFAULT'
                        } onChange={onChange} name="bloodrelationship" className='col-12 rounded border-primary my-3' >
                            <option value="DEFAULT" disabled >اختر واحدة</option>

                            {
                                optionClose.map((option,index) =>(
        ...
        <div className='my-3'>
            ...
            <div className="my-3">
                <h3 className="fs-2 text-secondary">الشخص الذي قام بتوجيه الطفل  إلى دار الأمومة</h3>
                <select value={
            ...
            <div className="my-3">
                <h3  className="fs-2 text-secondary">الرجوع إلى دار الأمومة بعد الفحص</h3>
                <div className="row my-3">
                    <label className="col-md-2 col-form-label fw-bold">التاريخ</label>
                    <div className="col-md-4">
